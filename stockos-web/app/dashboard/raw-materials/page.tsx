...
interface DerivedItem extends RawMaterialRow {
  total_stock: number;
...
export default function RawMaterialsPage() {
  ...
  // ── Fetch raw materials + inventory + vendor_items ──

  const { data: rawItems, isLoading } = useRealtimeQuery<RawMaterialRow[]>(
    ['raw-materials', userId ?? ''],
    'items',
    async () => {
    ...
    mutationFn: async (item: DerivedItem) => {
      ...
      const { data: po, error: poError } = await supabase
        ...
        .insert({
          ...
          notes: `Auto-reorder for ${item.standardized_name}`,
        })
        .select('id')
        .single();
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <SearchToolbar value={search} onChange={setSearch} placeholder="Search name or code…">
        <StatusBadge status="RAW_MATERIAL" />
      </SearchToolbar>
      ...
      <Card className="overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState title="No raw materials found" description='Add items with category "Raw Material" to see them here.' />
        ) : (
      ...
      <Dialog open={!!confirmId} onOpenChange={(open) => !open && setConfirmId(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Deactivate raw material?</DialogTitle>
